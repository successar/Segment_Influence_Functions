...
class ConditionalRandomField(torch.nn.Module):
    ...
    def token_conditional_likelihood(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        logits: (B, L, C)
        labels: (B, L)
        """
        likelihood = torch.zeros_like(logits)
        likelihood += logits 
        likelihood[:, :-1] += self.transitions[:, labels[:, 1:]].permute(1, 2, 0)
        likelihood[:, 1:] += self.transitions[labels[:, :-1], :].permute(0, 1, 2)
        likelihood = torch.nn.LogSoftmax(dim=-1)(likelihood)
        return likelihood
