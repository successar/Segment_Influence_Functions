...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(1, sequence_length):
            ...
            alpha = util.logsumexp(inner, 1) * mask[i].view(batch_size, 1) + alpha * (1 - mask[i]).view(
                batch_size, 1
            )
    ...
    def marginal_likelihood(self, logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        batch_size, sequence_length, num_tags = logits.size()
        logits = logits.transpose(0, 1).contiguous()  # (L, B, C)
        mask = mask.float().transpose(0, 1).contiguous().unsqueeze(-1)  # (L, B, 1)

        log_alphas = torch.zeros((sequence_length, batch_size, num_tags)).to(logits.device)
        log_betas = torch.zeros((sequence_length, batch_size, num_tags)).to(logits.device)

        log_alphas[0] = logits[0] * mask[0]

        for i in range(1, sequence_length):
            inner = (
                logits[i][:, None, :]
                + self.transitions[None, :, :]
                + log_alphas[i - 1][:, :, None]  # (B, C, C)
            )
            log_alphas[i] = util.logsumexp(inner, 1) * mask[i] + log_alphas[i - 1] * (1 - mask[i])

        for i in range(sequence_length - 2, -1, -1):
            inner = self.transitions[None, :, :] + logits[i + 1][:, None, :] + log_betas[i + 1][:, None, :]
            log_betas[i] = util.logsumexp(inner, 2) * mask[i + 1] + log_betas[i + 1] * (1 - mask[i + 1])

        potentials = log_alphas + log_betas  # (L, B, C)
        input_likelihood = util.logsumexp(log_alphas[-1], -1)
        log_marginals = potentials - input_likelihood[None, :, None]
        return log_marginals.transpose(0, 1)

    def forward(
        self, inputs: torch.Tensor, tags: torch.Tensor, mask: torch.ByteTensor = None, reduction: bool = True
    ) -> torch.Tensor:
        """
    ...
    def multi_tag_forward(
        self, inputs: torch.Tensor, tags: torch.Tensor, mask: torch.ByteTensor
    ) -> torch.Tensor:
        """
        Computes the log likelihood.
        tags: (B, variant, L)
        ...
        """
        # pylint: disable=arguments-differ
        tags = tags.transpose(0, 1)  # (variant, B, L)
        mask = mask.transpose(0, 1)  # (variant, B, L)

        log_denominator = self._input_likelihood(inputs, mask[0])
        log_numerator = []
        for v in range(tags.shape[0]):
            log_numerator.append(self._joint_likelihood(inputs, tags[v], mask[0]).unsqueeze(0))
        log_numerator = torch.cat(log_numerator, dim=0)
        log_numerator = log_numerator.masked_fill((~mask).all(-1), float("-inf"))
        log_numerator = torch.logsumexp(log_numerator, dim=0)
